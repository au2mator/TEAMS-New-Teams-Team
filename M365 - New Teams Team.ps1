...
try {
    ...
    try {
        Write-au2matorLog -Type INFO -Text "Try to get User ID of Owner"
        $TempOwner = (Get-ADUser -Identity $c_TeamOwner).UserPrincipalName

        $URLOwnwer = "https://graph.microsoft.com/v1.0/users/$TempOwner"
        $ResultOwner = Invoke-RestMethod -Headers $headers -Uri $URLOwnwer -Method Get

        Write-au2matorLog -Type INFO -Text "Found the following ID: $($ResultOwner.id)"

        try {
            Write-au2matorLog -Type INFO -Text "Try to create Team"
...
"@
            ...
            Write-au2matorLog -Type INFO -Text "Team creation triggered with Response: $($responseHeaders.'Content-Location')"

            Write-au2matorLog -Type INFO -Text "Check when Team Creation is succeeded"
            do {
                ...
                Write-au2matorLog -Type INFO -Text "Creation Status is: $($TeamStatus.Status)"

            } until ($TeamStatus.Status -eq "succeeded")
            Write-au2matorLog -Type INFO -Text "Team Creation succeeded"

            try {
                Write-au2matorLog -Type INFO -Text "Try to add Memebers to Team"

                $SplitMembers = $c_TeamMember.split(";")
                foreach ($Member in $SplitMembers) {
                    $TempMember = (Get-ADUser -Identity $Member).UserPrincipalName
                    Write-au2matorLog -Type INFO -Text "Work with User: $TempMember"
                    ...
                    Write-au2matorLog -Type INFO -Text "Found the following ID: $($ResultMember.id)"

                    $URLAddMember = "https://graph.microsoft.com/v1.0$($responseHeaders.'Content-Location')/members"
                    Write-au2matorLog -Type INFO -Text "URL: $URLAddMember"
...
"@
                    ...
                    Write-au2matorLog -Type INFO -Text "Added Member to the Team: $($C_TeamName)" 
                }

                $au2matorReturn = "The Team was created $($C_TeamName)"
                $AdditionalHTML = "The Team was created $($C_TeamName)
            <br>
            "
                $Status = "COMPLETED"
            }
                ...
                Write-au2matorLog -Type ERROR -Text "Could not add Members"
                Write-au2matorLog -Type ERROR -Text "Error: $Error"

                $au2matorReturn = "Could not add Members, Error: $Error"
                $AdditionalHTML = "Could not add Members
            <br>
            Error: $Error
            "
                $Status = "ERROR"
            ...
            Write-au2matorLog -Type ERROR -Text "Could not create the Team"
            Write-au2matorLog -Type ERROR -Text "Error: $Error"

            $au2matorReturn = "Could not create the Team, Error: $Error"
            $AdditionalHTML = "Could not create the Team
        <br>
        Error: $Error
        "
            $Status = "ERROR"
        ...
        Write-au2matorLog -Type ERROR -Text "Could not Query Owner for ID"
        Write-au2matorLog -Type ERROR -Text "Error: $Error"

        $au2matorReturn = "Could not Query Owner for ID, Error: $Error"
        $AdditionalHTML = "Could not Query Owner for ID
        <br>
        Error: $Error
            "
        $Status = "ERROR"
